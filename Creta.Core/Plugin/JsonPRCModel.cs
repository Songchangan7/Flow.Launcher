/* We basically follow the Json-RPC 2.0 spec (http://www.jsonrpc.org/specification) to invoke methods between Creta and other plugins, 
 ...
 *      Creta -------JsonRPCServerRequestModel--------> client
 *      Creta <------JsonRPCQueryResponseModel--------- client
 *      
 * When execute a action (which mean user select an item in reulst item):
 *      Creta -------JsonRPCServerRequestModel--------> client
 *      Creta <------JsonRPCResponseModel-------------- client
 * 
 */
...
namespace Creta.Core.Plugin
{
    ...
    /// <summary>
    /// Json RPC Request(in query response) that client sent to Creta
    /// </summary>
    ...
    /// <summary>
    /// Represent the json-rpc result item that client send to Creta
    /// Typically, we will send back this request model to client after user select the result item
    /// But if the request method starts with "Creta.", we will invoke the public APIs we expose.
    /// </summary>
