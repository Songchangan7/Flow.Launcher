...
#pragma warning restore SYSLIB0020 // Type or member is obsolete
        ...
        protected override async Task<bool> ExecuteResultAsync(JsonRPCResult result)
        {
            ...
            if (TryGetHostApiMethod(result.JsonRPCAction.Method, out var hostApiMethod))
            {
                ExecuteCretaAPI(hostApiMethod,
            ...
            else
            {
                ...
                if (TryGetHostApiMethod(jsonRpcRequestModel?.Method, out hostApiMethod))
                {
                    ExecuteCretaAPI(hostApiMethod,
        ...
        /// <summary>
        /// Accept both Creta.* and legacy Flow.Launcher.* host API prefixes so community plugins keep working.
        /// </summary>
        private static bool TryGetHostApiMethod(string method, out string apiMethod)
        {
            const string cretaPrefix = "Creta.";
            const string legacyPrefix = "Flow.Launcher.";

            if (!string.IsNullOrEmpty(method) && method.StartsWith(cretaPrefix, StringComparison.Ordinal))
            {
                apiMethod = method[cretaPrefix.Length..];
                return true;
            }

            if (!string.IsNullOrEmpty(method) && method.StartsWith(legacyPrefix, StringComparison.Ordinal))
            {
                apiMethod = method[legacyPrefix.Length..];
                return true;
            }
