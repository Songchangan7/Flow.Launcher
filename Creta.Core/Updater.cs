    ...
    public class Updater
    {
        ...
        public async Task UpdateAppAsync(bool silentUpdate = true)
        {
            ...
            try
            {
                if (!silentUpdate)
                    _api.ShowMsg(Localize.pleaseWait(),
                        Localize.update_creta_update_check());
                ...
                {
                    if (!silentUpdate)
                        _api.ShowMsgBox(Localize.update_creta_already_on_latest());
                    return;
                }

                if (!silentUpdate)
                    _api.ShowMsg(Localize.update_creta_update_found(),
                        Localize.update_creta_updating());
                ...
                if (DataLocation.PortableDataLocationInUse())
                {
                    ...
                    if (!FilesFolders.VerifyBothFolderFilesEqual(DataLocation.PortableDataPath, targetDestination,
                            (s) => _api.ShowMsgBox(s)))
                        _api.ShowMsgBox(Localize.update_creta_fail_moving_portable_user_profile_data(DataLocation.PortableDataPath, targetDestination));
                }
                else
                {
                ...
                if (_api.ShowMsgBox(newVersionTips, Localize.update_creta_new_update(),
            ...
            catch (Exception e)
            {
                ...
                if (!silentUpdate)
                    _api.ShowMsgError(Localize.update_creta_fail(),
                        Localize.update_creta_check_connection());
            }
            finally
            {
                UpdateLock.Release();
