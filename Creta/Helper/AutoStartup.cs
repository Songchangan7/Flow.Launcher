...
public class AutoStartup
{
    ...
    private const string LogonTaskName = $"{Constant.Creta} Startup";
    private const string LogonTaskDesc = $"{Constant.Creta} Auto Startup";

    public static void CheckIsEnabled(bool useLogonTaskForStartup)
    {
        // We need to check both because if both of them are enabled,
        // Hide Creta on startup will not work since the later one will trigger main window show event
    ...
    private static bool CheckRegistry()
    {
        try
        {
            ...
            if (key != null)
            {
                // Check if the action is the same as the current executable path
                // If not, we need to unschedule and reschedule the task
                var action = (key.GetValue(Constant.Creta) as string) ?? string.Empty;
    ...
    {
        using var key = Registry.CurrentUser.OpenSubKey(StartupPath, true);
        key?.DeleteValue(Constant.Creta, false);
        return true;
    }
    ...
    {
        using var key = Registry.CurrentUser.OpenSubKey(StartupPath, true);
        key?.SetValue(Constant.Creta, $"\"{Constant.ExecutablePath}\"");
        return true;
    }
}
