...
using Creta.Plugin.BrowserBookmark.Commands;
using Creta.Plugin.BrowserBookmark.Models;
using Creta.Plugin.BrowserBookmark.Views;
...
public class Main : ISettingProvider, IPlugin, IReloadable, IPluginI18n, IContextMenu, IDisposable
{
    ...
    {
        return Localize.creta_plugin_browserbookmark_plugin_name();
    }

    public string GetTranslatedPluginDescription()
    {
        return Localize.creta_plugin_browserbookmark_plugin_description();
    }
    ...
    public List<Result> LoadContextMenus(Result selectedResult)
    {
        ...
        return new List<Result>()
        {
            new()
            {
                Title = Localize.creta_plugin_browserbookmark_copyurl_title(),
                SubTitle = Localize.creta_plugin_browserbookmark_copyurl_subtitle(),
                Action = _ =>
                {
                    try
                    {
                    ...
                    catch (Exception e)
                    {
                        ...
                        Context.API.ShowMsgError(Localize.creta_plugin_browserbookmark_copy_failed());
                        return false;
                    }
                },
