...
using Creta.Plugin.Calculator.Views;
using Mages.Core;
...
namespace Creta.Plugin.Calculator
{
    public class Main : IPlugin, IPluginI18n, ISettingProvider
    {
            ...
            try
            {
                ...
                if (result == null || string.IsNullOrEmpty(result.ToString()))
                {
                    ...
                    [
                        new Result
                        {
                            Title = Localize.creta_plugin_calculator_expression_not_complete(),
                            IcoPath = IcoPath
                        }
                    ];
                }
                ...
                {
                    result = Localize.creta_plugin_calculator_not_a_number();
                }

                if (result is Function)
                {
                    result = Localize.creta_plugin_calculator_expression_not_complete();
                }
                ...
                if (!string.IsNullOrEmpty(result.ToString()))
                {
                    ...
                    [
                        new Result
                        {
                            ...
                            SubTitle = Context == null ? string.Empty : Localize.creta_plugin_calculator_copy_number_to_clipboard(),
                            CopyText = newResult,
                            Action = c =>
                            {
                                try
                                ...
                                {
                                    Context.API.ShowMsgBox(Localize.creta_plugin_calculator_failed_to_copy());
                                    return false;
                                }
                            }
                        }
        ...
        {
            return Localize.creta_plugin_calculator_plugin_name();
        }

        public string GetTranslatedPluginDescription()
        {
            return Localize.creta_plugin_calculator_plugin_description();
        }
