...
public static class EverythingDownloadHelper
{
    public static async Task<string> PromptDownloadIfNotInstallAsync(string installedLocation, IPublicAPI api)
    {
        ...
        {
            if (api.ShowMsgBox(
                Localize.creta_plugin_everything_installing_select(Environment.NewLine),
                Localize.creta_plugin_everything_installing_title(),
        ...
        api.ShowMsg(Localize.creta_plugin_everything_installing_title(),
            Localize.creta_plugin_everything_installing_subtitle(), "", useMainWindowAsOwner: false);

        await DroplexPackage.Drop(App.Everything1_4_1_1009).ConfigureAwait(false);

        api.ShowMsg(Localize.creta_plugin_everything_installing_title(),
            Localize.creta_plugin_everything_installationsuccess_subtitle(), "", useMainWindowAsOwner: false);
