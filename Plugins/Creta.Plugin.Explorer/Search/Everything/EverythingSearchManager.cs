...
using Creta.Plugin.Explorer.Exceptions;
using Creta.Plugin.Explorer.Search.IProvider;
    ...
    public class EverythingSearchManager : IIndexProvider, IContentIndexProvider, IPathIndexProvider
    {
        ...
        private async ValueTask ThrowIfEverythingNotAvailableAsync(CancellationToken token = default)
        {
            try
            {
                if (!await EverythingApi.IsEverythingRunningAsync(token))
                    throw new EngineNotAvailableException(
                        ...
                        Localize.creta_plugin_everything_click_to_launch_or_install(),
                        Localize.creta_plugin_everything_is_not_running(),
            ...
            catch (DllNotFoundException)
            {
                throw new EngineNotAvailableException(
                    ...
                    Localize.creta_plugin_everything_sdk_issue());
            }
        }
        ...
        private async ValueTask<bool> ClickToInstallEverythingAsync(ActionContext _)
        {
            try
            {
                ...
                {
                    Main.Context.API.ShowMsgError(Localize.creta_plugin_everything_not_found());
            ...
            {
                Main.Context.API.ShowMsgError(Localize.creta_plugin_everything_install_issue());
            ...
            [EnumeratorCancellation] CancellationToken token)
        {
            ...
            if (!Settings.EnableEverythingContentSearch)
            {
                throw new EngineNotAvailableException(Enum.GetName(Settings.IndexSearchEngineOption.Everything)!,
                    Localize.creta_plugin_everything_enable_content_search(),
                    Localize.creta_plugin_everything_enable_content_search_tips(),
                    Constants.EverythingErrorImagePath,
                    _ =>
                    {
