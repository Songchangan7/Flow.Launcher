...
using Creta.Plugin.ProcessKiller.Views;
...
namespace Creta.Plugin.ProcessKiller
{
    public class Main : IPlugin, IPluginI18n, IContextMenu, ISettingProvider
    {
        ...
        {
            return Localize.creta_plugin_processkiller_plugin_name();
        }

        public string GetTranslatedPluginDescription()
        {
            return Localize.creta_plugin_processkiller_plugin_description();
        }
        ...
        public List<Result> LoadContextMenus(Result result)
        {
            ...
            if (similarProcesses.Any())
            {
                ...
                {
                    Title = Localize.creta_plugin_processkiller_kill_instances(),
        ...
        private List<Result> CreateResultsFromQuery(Query query)
        {
            ...
            foreach (var pr in processlist)
            {
                ...
                results.Add(new Result()
                {
                    ...
                    AutoCompleteText = $"{Context.CurrentPluginMetadata.ActionKeyword}{Flow.Launcher.Plugin.Query.TermSeparator}{p.ProcessName}",
                    Action = (c) =>
                    {
                        ProcessHelper.TryKill(p);
            ...
            if (processlist.Count > 1 && !string.IsNullOrEmpty(searchTerm) && sortedResults.All(r => r.SubTitle == firstResult?.SubTitle))
            {
                sortedResults.Insert(1, new Result()
                {
                    ...
                    Title = Localize.creta_plugin_processkiller_kill_all(firstResult?.ContextData),
                    SubTitle = Localize.creta_plugin_processkiller_kill_all_count(processlist.Count),
                    Score = 200,
                    Action = (c) =>
                    {
