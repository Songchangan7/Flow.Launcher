...
namespace Creta.Plugin.Program.Logger
{
    /// <summary>
    /// The Program plugin has seen many issues recorded in the Creta repo related to various loading of Windows programs.
    ...
    internal static class ProgramLogger
    {
            ...
            string interpretationMessage, Exception e)
        {
            ...
            logger.Error("------------- BEGIN Creta.Plugin.Program exception -------------");

            do
            {
                if (IsKnownWinProgramError(e, callingMethodName) || IsKnownUWPProgramError(e, callingMethodName))
                {
                    possibleResolution = "Can be ignored and Creta should still continue, however the program may not be loaded";
                    errorStatus = "KNOWN";
                }
            ...
            logger.Error("------------- END Creta.Plugin.Program exception -------------");
        }
        ...
        /// => Example: |Win32|LnkProgram|c:\..\chrome.exe|Permission denied on directory, but Creta should continue
        /// </summary>
        [MethodImpl(MethodImplOptions.Synchronized)]
