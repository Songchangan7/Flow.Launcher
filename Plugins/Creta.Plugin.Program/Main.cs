...
using Creta.Plugin.Program.Views;
using Creta.Plugin.Program.Views.Models;
using Flow.Launcher.Plugin.SharedCommands;
using Microsoft.Extensions.Caching.Memory;
using Path = System.IO.Path;

namespace Creta.Plugin.Program
{
    public class Main : ISettingProvider, IAsyncPlugin, IPluginI18n, IContextMenu, IAsyncReloadable, IDisposable
    {
        ...
        public async Task InitAsync(PluginInitContext context)
        {
            ...
            await Context.API.StopwatchLogInfoAsync(ClassName, "Preload programs cost", async () =>
            {
                ...
                // If plugin cache directory is this: D:\\Data\\Cache\\Plugins\\Creta.Plugin.Program
        ...
        {
            return Context.API.GetTranslation("creta_plugin_program_plugin_name");
        }

        public string GetTranslatedPluginDescription()
        {
            return Context.API.GetTranslation("creta_plugin_program_plugin_description");
        }
        ...
        public List<Result> LoadContextMenus(Result selectedResult)
        {
            ...
            menuOptions.Add(
                new Result
                {
                    Title = Context.API.GetTranslation("creta_plugin_program_disable_program"),
                    Action = c =>
                    {
                        _ = Task.Run(async () =>
                        {
                            try
                            {
                                ...
                                if (disabled)
                                {
                                    ResetCache();
                                    Context.API.ShowMsg(
                                        Context.API.GetTranslation("creta_plugin_program_disable_dlgtitle_success"),
                                        Context.API.GetTranslation(
                                            "creta_plugin_program_disable_dlgtitle_success_message"));
                                }
                                Context.API.ReQuery();
                            }
                            catch (Exception e)
        ...
        public static void StartProcess(Func<ProcessStartInfo, Process> runProcess, ProcessStartInfo info)
        {
            ...
            {
                var title = Context.API.GetTranslation("creta_plugin_program_disable_dlgtitle_error");
                var message = string.Format(Context.API.GetTranslation("creta_plugin_program_run_failed"),
                    info.FileName);
