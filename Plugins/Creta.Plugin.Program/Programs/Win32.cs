...
using Creta.Plugin.Program.Logger;
...
namespace Creta.Plugin.Program.Programs
{
    [MemoryPackable]
    public partial class Win32 : IProgram, IEquatable<Win32>
    {
        ...
        public List<Result> ContextMenus(IPublicAPI api)
        {
            var contextMenus = new List<Result>
            {
                new()
                {
                    Title = api.GetTranslation("creta_plugin_program_run_as_different_user"),
                    Action = c =>
                    {
                        var info = new ProcessStartInfo
                        {
                ...
                new()
                {
                    Title = api.GetTranslation("creta_plugin_program_run_as_administrator"),
                    Action = c =>
                    {
                        var info = new ProcessStartInfo
                        {
                ...
                new()
                {
                    Title = api.GetTranslation("creta_plugin_program_open_containing_folder"),
                    Action = _ =>
                    {
                        Main.Context.API.OpenDirectory(ParentDirectory, FullPath);
        ...
        private Result OpenTargetFolderContextMenuResult(IPublicAPI api)
        {
            return new Result
            {
                Title = api.GetTranslation("creta_plugin_program_open_target_folder"),
                Action = _ =>
                {
