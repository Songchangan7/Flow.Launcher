...
using Creta.Plugin.Shell.Views;
using WindowsInput;
using WindowsInput.Native;
using Control = System.Windows.Controls.Control;
using Keys = System.Windows.Forms.Keys;

namespace Creta.Plugin.Shell
{
    public class Main : IPlugin, ISettingProvider, IPluginI18n, IContextMenu, IDisposable
    {
        ...
        private List<Result> GetHistoryCmds(string cmd, Result result)
        {
            IEnumerable<Result> history = _settings.CommandHistory.Where(o => o.Key.Contains(cmd))
                ...
                .Select(m =>
                {
                    if (m.Key == cmd)
                    {
                        result.SubTitle = Localize.creta_plugin_cmd_cmd_has_been_executed_times(m.Value);
                        return null;
                    }
                    ...
                    {
                        Title = m.Key,
                        SubTitle = Localize.creta_plugin_cmd_cmd_has_been_executed_times(m.Value),
                        IcoPath = Image,
                        Action = c =>
                        {
                            var runAsAdministrator =
        ...
        private Result GetCurrentCmd(string cmd)
        {
            Result result = new Result
            {
                Title = cmd,
                Score = 5000,
                SubTitle = Localize.creta_plugin_cmd_execute_through_shell(),
                IcoPath = Image,
                Action = c =>
                {
                    var runAsAdministrator =
        ...
        private ProcessStartInfo PrepareProcessStartInfo(string command, bool runAsAdministrator = false)
        {
            ...
            var notifyStr = Localize.creta_plugin_cmd_press_any_key_to_close();
        ...
        private void Execute(Func<ProcessStartInfo, Process> startProcess, ProcessStartInfo info)
        {
            ...
            catch (FileNotFoundException e)
            {
                Context.API.ShowMsgError(GetTranslatedPluginTitle(),
                    Localize.creta_plugin_cmd_command_not_found(e.Message));
            }
            catch (Win32Exception e)
            {
                Context.API.ShowMsgError(GetTranslatedPluginTitle(),
                    Localize.creta_plugin_cmd_error_running_command(e.Message));
            }
            catch (Exception e)
            {
        ...
        private static void OnWinRPressed()
        {
            ...
            _ = Task.Run(async () =>
            {
                Context.API.ChangeQuery($"{Context.CurrentPluginMetadata.ActionKeywords[0]}{Flow.Launcher.Plugin.Query.TermSeparator}");
        ...
        {
            return Localize.creta_plugin_cmd_plugin_name();
        }

        public string GetTranslatedPluginDescription()
        {
            return Localize.creta_plugin_cmd_plugin_description();
        }
        ...
        public List<Result> LoadContextMenus(Result selectedResult)
        {
            var results = new List<Result>
            {
                new()
                {
                    Title = Localize.creta_plugin_cmd_run_as_different_user(),
                    Action = c =>
                    {
                ...
                new()
                {
                    Title = Localize.creta_plugin_cmd_run_as_administrator(),
                    Action = c =>
                    {
                ...
                new()
                {
                    Title = Localize.creta_plugin_cmd_copy(),
                    Action = c =>
                    {
