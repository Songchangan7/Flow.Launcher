    ...
    public static class ThemeSelector
    {
        ...
        private static Result CreateThemeResult(ThemeData theme, ThemeData selectedTheme, int score, IList<int> highlightData)
        {
            ...
            if (theme.IsDark == true)
            {
                ...
                {
                    description = Localize.creta_plugin_sys_type_isdark_hasblur();
                }
                else
                {
                    description = Localize.creta_plugin_sys_type_isdark();
                }
            }
            else
            {
                if (theme.HasBlur == true)
                {
                    description = Localize.creta_plugin_sys_type_hasblur();
                }
                else
                {
