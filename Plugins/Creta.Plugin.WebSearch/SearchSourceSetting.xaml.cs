...
namespace Creta.Plugin.WebSearch
{
    public partial class SearchSourceSettingWindow
    {
        ...
        private void OnConfirmButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                var warning = _api.GetTranslation("creta_plugin_websearch_input_title");
                _context.API.ShowMsgBox(warning);
            }
            else if (string.IsNullOrEmpty(_searchSource.Url))
            {
                var warning = _api.GetTranslation("creta_plugin_websearch_input_url");
                _context.API.ShowMsgBox(warning);
            }
            else if (string.IsNullOrEmpty(_searchSource.ActionKeyword))
            {
                var warning = _api.GetTranslation("creta_plugin_websearch_input_action_keyword");
                _context.API.ShowMsgBox(warning);
            }
        ...
        private void EditSearchSource()
        {
            ...
            if (!_context.API.ActionKeywordAssigned(newKeyword) || oldKeyword == newKeyword)
            {
                ...
                {
                    var warning = _api.GetTranslation("creta_plugin_websearch_edit_failed");
                    _context.API.ShowMsgBox(warning);
                    Close();
                }
            }
            ...
            if (result == true)
            {
                ...
                if (!string.IsNullOrEmpty(selectedNewIconImageFullPath))
                {
                    if (_viewModel.ShouldProvideHint(selectedNewIconImageFullPath))
                        _context.API.ShowMsgBox(_api.GetTranslation("creta_plugin_websearch_iconpath_hint"));
