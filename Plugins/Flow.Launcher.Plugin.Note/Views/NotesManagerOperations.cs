...
internal static class NotesManagerOperations
{
    internal static bool TryEditNote(NoteRepository repository, NoteItem note)
    {
        ...
        ShowOperationError(Localize.flowlauncher_plugin_note_error_update_title(), errorMessage);
        return false;
    }
    ...
    internal static bool TryDeleteNotes(NoteRepository repository, IReadOnlyList<NoteItem> notes)
    {
        ...
        var message = notes.Count == 1
            ? Localize.flowlauncher_plugin_note_delete_confirm_message(
                Environment.NewLine,
                NotePresentation.BuildSavedSubtitle(notes[0].Content))
            : Localize.flowlauncher_plugin_note_settings_notes_batch_delete_confirm_message(notes.Count);
        ...
        foreach (var note in notes)
        {
            ...
            {
                ShowOperationError(Localize.flowlauncher_plugin_note_error_delete_title(), errorMessage);
                return false;
            }
        }
    ...
    internal static bool TryArchiveNotes(NoteRepository repository, IReadOnlyList<NoteItem> notes)
    {
        ...
        {
            var result = Main.Context.API.ShowMsgBox(
                Localize.flowlauncher_plugin_note_settings_notes_batch_archive_confirm_message(notes.Count),
                Localize.flowlauncher_plugin_note_delete_confirm_caption(),
                MessageBoxButton.YesNo,
    ...
    internal static bool TryBatchAddTags(NoteRepository repository, IReadOnlyList<NoteItem> notes)
    {
        ...
        var window = new NoteTagsEditorWindow(
            string.Empty,
            Localize.flowlauncher_plugin_note_settings_notes_batch_tags_editor_title(),
            Localize.flowlauncher_plugin_note_settings_notes_batch_tags_editor_hint(notes.Count),
            Localize.flowlauncher_plugin_note_settings_notes_batch_tags_editor_save());
        if (window.ShowDialog() != true)
        {
            return false;
        }
    ...
    internal static void TryOpenNotesFileFolder(NoteRepository repository)
    {
        ...
        {
            ShowOperationError(
                Localize.flowlauncher_plugin_note_settings_notes_open_folder_failed_title(),
                ex.Message);
        }
    }
