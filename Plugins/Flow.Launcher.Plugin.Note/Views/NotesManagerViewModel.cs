...
internal sealed class NotesManagerViewModel : BaseModel
{
    ...
    internal void LoadFromRepository(NoteRepository repository)
    {
        ...
        SummaryText = Localize.flowlauncher_plugin_note_settings_notes_summary(
            allNotes.Count,
            archivedCount,
    ...
    private void RefreshVisiblePage()
    {
        ...
        if (_filteredRows.Count == 0)
        {
            PageInfoText = HasActiveFilters()
                ? Localize.flowlauncher_plugin_note_settings_notes_filter_empty(_totalNoteCount)
                : Localize.flowlauncher_plugin_note_settings_notes_empty();
        ...
        PageInfoText = HasActiveFilters()
            ? Localize.flowlauncher_plugin_note_settings_notes_page_info_filtered(
                _currentPage,
                _totalPages,
                _filteredRows.Count,
                _totalNoteCount)
            : Localize.flowlauncher_plugin_note_settings_notes_page_info(
                _currentPage,
                _totalPages,
                _filteredRows.Count);
        OnPropertyChanged(nameof(CanGoPrevious));
