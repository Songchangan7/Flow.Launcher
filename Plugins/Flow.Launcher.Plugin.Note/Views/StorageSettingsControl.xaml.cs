...
public partial class StorageSettingsControl : UserControl
{
    ...
    private void BrowseButton_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        {
            Title = Localize.flowlauncher_plugin_note_settings_storage_browse_dialog_title(),
            Filter = Localize.flowlauncher_plugin_note_settings_storage_browse_filter(),
            DefaultExt = ".json",
            AddExtension = true,
    ...
    private void SaveButton_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        if (result.Succeeded)
        {
            Main.Context.API.ShowMsg(
                Localize.flowlauncher_plugin_note_settings_storage_saved_title(),
                BuildSuccessMessage(result));
            return;
        }

        Main.Context.API.ShowMsgError(
            Localize.flowlauncher_plugin_note_settings_storage_save_failed_title(),
            result.ErrorMessage);
    }
    ...
    {
        return Localize.flowlauncher_plugin_note_settings_storage_active_path(path);
    }
    ...
    private static string BuildSuccessMessage(NoteStorageChangeResult result)
    {
        ...
        {
            return Localize.flowlauncher_plugin_note_settings_storage_success_unchanged(result.CurrentNoteCount);
        }

        if (result.NotesMerged)
        {
            return Localize.flowlauncher_plugin_note_settings_storage_success_merged(
                result.MigratedNoteCount,
                result.ExistingTargetNoteCount,
                result.CurrentNoteCount);
        }

        return Localize.flowlauncher_plugin_note_settings_storage_success_migrated(
            result.MigratedNoteCount,
            result.CurrentNoteCount);
    }
