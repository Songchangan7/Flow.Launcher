...
- `Flow Launcher` 是一个基于 Windows 的快速启动器与统一搜索入口。
...
- UI 框架：`WPF`
- 目标框架：
  - 主程序 `Flow.Launcher`：`net9.0-windows10.0.19041.0`
...
```mermaid
flowchart LR
    A["用户输入 / 全局热键"] --> B["Flow.Launcher 主程序"]
...
- 应用入口位于 `Flow.Launcher/App.xaml.cs` 的 `Main()`。
- 启动时首先通过 `FlowLauncherJsonStorage<Settings>` 加载用户设置。
...
- `Flow.Launcher/MainWindow.xaml.cs` 负责主窗口行为层逻辑。
- 主要职责包括：
  - 绑定 `MainViewModel`
  - 初始化窗口位置与尺寸
  - 管理托盘图标和右键菜单
...
- 若不存在有效便携目录，则使用漫游目录：`%APPDATA%\FlowLauncher`
...
- 特点包括：
  ...
  - 支持 `.bak` 备份恢复
  - 使用原子替换降低设置文件损坏风险
- `FlowLauncherJsonStorage<T>` 是面向主程序设置体系的封装，负责结合 `DataLocation` 自动确定文件位置。
...
- 插件目录：`Plugins/Flow.Launcher.Plugin.LocalPromptSearch`
- 动作关键字：`pt`
- 目标：
  - 在本地维护 Prompt 模板库
  - 在 Flow Launcher 中快速检索、复制并可选回焦粘贴
- 核心设计：
...
- 插件目录：`Plugins/Flow.Launcher.Plugin.BrowserTab`
- 动作关键字：`tab`
- 目标：
...
- 插件目录：`Plugins/Flow.Launcher.Plugin.Note`
- 动作关键字：`n`
- 目标：
  - 提供轻量、快速、键盘友好的临时笔记能力
- 核心设计：
...
- 当前价值：
  ...
  - 与 Flow Launcher 的键盘流操作模式高度一致

## 15. 测试设计

- 测试项目位于 `Flow.Launcher.Test`
- 当前可见测试覆盖方向包括：
  - `QueryBuilderTest`
  - `FuzzyMatcherTest`
  - `PluginLoadTest`
  ...
  - `NoteRepositoryTests`
- 说明：
  - 测试项目已显式引用 `Flow.Launcher.Plugin.Note`，说明 Quick Note 已纳入自动化验证范围
...
- 主程序、核心库和基础设施库默认输出到：
  - `Output\Debug`
  - `Output\Release`
- 主程序在构建前会尝试结束正在运行的 `Flow.Launcher.exe`，避免文件占用。
...
- 想调整数据目录或持久化行为：
  - 重点关注 `DataLocation`、`JsonStorage<T>`、`FlowLauncherJsonStorage<T>`。
- 想调整主题或窗口行为：
...
- 当前仓库延续了 Flow Launcher 以插件为中心的宿主架构。
